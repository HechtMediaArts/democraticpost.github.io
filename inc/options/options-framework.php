...
/**
*
* // ######################## Besagt geänderte Punkte im Originalscript. #####################
*/
...
function optionsframework_init() {
	...
	if ( $optionsfile = locate_template( array('options/options.php') ) ) {
...
/**
 * Create Admin Menu Separator - useless function, but pretty cool :-)
 **/
...
function admin_menu_separator() {
	...
	add_admin_menu_separator(30.7465);
	add_admin_menu_separator(58.9999);
}

/* Add a top-level menu page called  Evolution "Theme Options" to the appearance menu. */
...
global $data;
	...
	function optionsframework_add_page() {
		...
		$themename =  $theme_data;
		$icon = get_template_directory_uri() . '/images/favicon.ico';
	
 				$of_page = add_menu_page ( 'Page Title', $themename, 'edit_theme_options','evolution-options', '', $icon, 58.996);
          
				$of_page = add_submenu_page( 'evolution-options', 'Theme Optionen', 'Theme Optionen', 'edit_theme_options', 'evolution-options', 'optionsframework_page' ); // Default

				// Die Seite für die SEO-Optionen zum Adminmenü hinzufügen
				$seo_page = add_submenu_page('evolution-options', __('SEO Optionen','menu-test'), __('SEO Optionen','menu-test'), 'edit_theme_options', 'seo-options', 'optionsframework_options_page');

		// Load the required CSS and javscript
		add_action( 'admin_enqueue_scripts', 'optionsframework_load_scripts' );
		add_action( 'admin_print_styles-' . $of_page, 'optionsframework_load_styles' );
		add_action('admin_print_styles-' . $seo_page, 'evolution_seo_admin_css');
...
function evolution_seo_admin_css() {

wp_enqueue_style( 'seo-style', get_bloginfo('template_directory') . '/inc/seo-functions/admin/admin-style.css' );
}
...
function optionsframework_load_scripts( $hook ) {
	...
	//if ( 'appearance_page_evolution-options' != $hook )			// Auskommentiert, da anderer Menüpunkt
       // return;
...
?>
    ...
    <div id="optionsframework-metabox" class="metabox-holder">
	    <div id="optionsframework" class="postbox">
			...
			<div class="optionsframework-submit">
				<input type="submit" class="button-primary" name="update" value="<?php esc_attr_e( 'Optionen Speichern', 'options_framework_theme' ); ?>" />
				<input type="submit" class="reset-button button-secondary" name="reset" value="<?php esc_attr_e( 'Optionen zurücksetzen', 'options_framework_theme' ); ?>" onclick="return confirm( '<?php print esc_js( __( 'Click OK to reset. Any theme settings will be lost!', 'options_framework_theme' ) ); ?>' );" />
				<div class="clear"></div>
			</div>	
...
function optionsframework_validate( $input ) {
	...
	if ( isset( $_POST['reset'] ) ) {
		add_settings_error( 'evolution-options', 'restore_defaults', __( 'Default options restored.', 'options_framework_theme' ), 'updated fade' );
...
function optionsframework_save_options_notice() {
	add_settings_error( 'evolution-options', 'save_options', __( 'Options saved.', 'options_framework_theme' ), 'updated fade' );
}
...
function optionsframework_adminbar() {
	...
	$wp_admin_bar->add_menu( array(
			...
			'href' => admin_url( 'themes.php?page=evolution-options' )
		));
}
